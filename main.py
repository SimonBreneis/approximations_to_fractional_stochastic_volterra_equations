...
if __name__ == '__main__':
    ...
    '''
    print('Hello')
    params = {'H': np.array([0.15]), 'lambda': np.array([1.0]), 'rho': np.array([-0.8]), 'nu': np.array([0.2]),
              'theta': np.array([0.01]), 'V_0': np.array([0.01]), 'T': np.array([0.2]), 'K': np.exp(np.linspace(-1.0, 0.5, 301)*np.sqrt(0.2))}
...
params = {'K': np.exp(k)}
print(optimize_kernel_approximation_for_simulation(params=params, N=1, N_time=1024, vol_behaviour='sticky', true_smile=true_smile, test=True))
'''
