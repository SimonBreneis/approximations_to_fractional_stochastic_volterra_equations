...
'''
print('Finished')
for N_times in 2 ** np.arange(12):

    tic = time.perf_counter()
    compute_final_rHeston_stock_prices(params='simple', Ns=np.array([3]), N_times=N_times, modes=['european'], vol_behaviours=['sticky'], recompute=True, m=20000000)
    print(time.perf_counter() - tic)
    tic = time.perf_counter()
    compute_final_rHeston_stock_prices(params='simple', Ns=np.array([3]), N_times=N_times, modes=['european'], vol_behaviours=['mackevicius random standard standard'], recompute=True, m=20000000)
    print(time.perf_counter() - tic)
    tic = time.perf_counter()
    compute_final_rHeston_stock_prices(params='simple', Ns=np.array([3]), N_times=N_times, modes=['european'], vol_behaviours=['mackevicius sequential standard standard'], recompute=True, m=20000000)
    print(time.perf_counter() - tic)
    tic = time.perf_counter()
    compute_final_rHeston_stock_prices(params='simple', Ns=np.array([3]), N_times=N_times, modes=['european'], vol_behaviours=['mackevicius sequential antithetic standard'], recompute=True, m=20000000)
    print(time.perf_counter() - tic)
    tic = time.perf_counter()
    compute_final_rHeston_stock_prices(params='simple', Ns=np.array([3]), N_times=N_times, modes=['european'], vol_behaviours=['mackevicius sequential standard control variate'], recompute=True, m=20000000)
    print(time.perf_counter() - tic)
    tic = time.perf_counter()
    compute_final_rHeston_stock_prices(params='simple', Ns=np.array([3]), N_times=N_times, modes=['european'], vol_behaviours=['mackevicius sequential antithetic control variate'], recompute=True, m=20000000)
    print(time.perf_counter() - tic)
...
k = np.sqrt(1) * np.linspace(-1.5, 0.75, 451)[220:-70:25]# [280:-140:10]# [220:-70:5]
params = {'K': np.exp(k), 'T': 1.}
params = rHeston_params(params)
true_smile = Data.true_iv_surface_eur_call[-1, 220:-70:25]
print(k, len(k))
...
compute_smiles_given_stock_prices(params=params, Ns=np.array([3]), N_times=2 ** np.arange(10), modes=['european'], vol_behaviours=['sticky', 'mackevicius random standard standard', 'mackevicius sequential standard standard', 'mackevicius sequential antithetic standard'], plot=True, true_smile=true_smile)
